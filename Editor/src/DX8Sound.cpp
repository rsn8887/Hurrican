...
// gespielt, damit es m�glich ist, einen Sound "�berlagernd" zu spielen
//
...
DirectSoundClass::DirectSoundClass(void)
{
	...
	// Zu Beginn alle Channels "verf�gbar" machen, da ja kein einziger Sound spielt
...
bool DirectSoundClass::LoadWave(char *filename, unsigned int nr)
{
	...
	if (wavefile == NULL)							// Fehler beim �ffnen ?
	{
		strcpy(Temp, "\n-> Wave laden Fehler : ");
...
// Zum Schlu� wird der Buffer mit den Daten gef�llt:
...
// --------------------------------------------------------------------------------------
// Wavedatei an Array-Position "nr" mit bestimmter Lautst�rke und Panning spielen
// --------------------------------------------------------------------------------------

bool DirectSoundClass::PlayWave(int Volume, int Panning, int nr)
{
	if(Volume == 0)					// h�rt man den Sound �berhaupt ? :)
		return false;

	// Channels durchgehen und pr�fen, welcher frei ist
	int i=0;
	...
	Channels[i]->SetVolume((DWORD)(-30*(100 - Volume)));			// Lautst�rke setzen
...
// --------------------------------------------------------------------------------------
// Channels auf fertige Sounds pr�fen und ggf. den Channel wieder freimachen
// --------------------------------------------------------------------------------------
...
void DirectSoundClass::UpdateChannels(void)
{
	...
	for(int i=0; i<MAX_CHANNELS; i++)
		if(ChannelStats[i] == 1)
		{
			...
			{
				ChannelStats[i] = 0;					// Dann erneut verf�gbar machen
...
// --------------------------------------------------------------------------------------
// aktuelle Sound-Lautst�rke abfragen
// --------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------
// aktuelle Musik-Lautst�rke abfragen
// --------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------
// Zur�ckliefern, wieviele Channels derzeit genutzt werden
// --------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------
// Alle Songs auf aktuelle Lautst�rke setzen
// --------------------------------------------------------------------------------------
