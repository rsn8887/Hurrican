...
char					TastaturPuffer[256];	// Tastaturpuffer f�r Keyboardabfrage
...
DirectInputClass::DirectInputClass(void)
{
	...
	// Zu Beginn alle Eingabeger�te zur�cksetzen
	MausX		 = 0;
	MausY		 = 0;
...
//----- Keyboard
	...
	// Datenformat f�r Keyboard festlegen
...
//----- Maus
	...
	// Datenformat f�r die Maus festlegen
...
//----- Joystick
	...
	if(Joystick)
	{
		...
		Protokoll.WriteText("Joystick : kein Joystick gefunden, �berspringe Initialisierung !\n", false);
...
bool DirectInputClass::MausUpdate(bool gepuffert)
{
   ...
   bool         fertig = false;
	...
	if (gepuffert == true) 
	{
		...
		if (dwElemente == 0)			// Keine Elemente wurden ver�ndert
			fertig = true;
         
		switch (od.dwOfs)				// Feld 'dwOfs' enth�lt Maus-Aktion:
		{		
            ...
            case DIMOFS_BUTTON0:		// Knopf 0 gedr�ckt
				if (od.dwData & 0x80)	// Knopf gedr�ck
				{
					MausButtons[0] = true;
					fertig = TRUE;
				}
				...
				case DIMOFS_BUTTON1:	// Knopf 1 gedr�ckt
				if (od.dwData & 0x80)	// Knopf gedr�ck
				{
					MausButtons[1] = true;
					fertig = TRUE;
				...
				case DIMOFS_BUTTON2:	// Knopf 2 gedr�ckt
				if (od.dwData & 0x80)	// Knopf gedr�ck
				{
					MausButtons[2] = true;
					fertig = TRUE;
