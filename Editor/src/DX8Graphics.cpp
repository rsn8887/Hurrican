                           ...
                           DWORD dwZ_Bits, DWORD dwStencil_Bits,bool VSync)
{
	...
	PresentParams.BackBufferHeight					= SCREENHEIGHT;					// Screenh�he
...
_ModeFound:
   ...
   /* Jetzt haben wir die Informationen zusammen und machen weiter */
	...
	// Device kann nur Texturen mit 2er-Potenz-Gr�sse
	if (d3dCaps.TextureCaps | D3DPTEXTURECAPS_POW2)
		PowerOfTwo = true; 
	else
		PowerOfTwo = false;

   // Globale Variable mit dem tats�chlichen BackBuffer f�llen
...
bool DirectGraphicsClass::TakeScreenshot(char Filename[100])
{
	...
	Bits = new BYTE[SCREENWIDTH * SCREENHEIGHT * 3 + 1];		// Array mit Pixeldaten erstellen (f�r die Konvertierung in 24 bit)
	...
	// BMP Datei erzeugen, wobei bereits existierende Dateien nicht �berschrieben werden
	...
	// Datei neu erzeugen und zum schreiben �ffnen
	f = fopen(TempName, "wb");
	if (!f)
	{
...
bool DirectGraphicsClass::PutPixel(int x, int y, UCHAR rot,UCHAR gruen, UCHAR blau, LPDIRECT3DSURFACE8 lpDDSurf)
{
	...
	int DDS_Hoehe;				// H�he der Surface
	USHORT *Vram;				// Speicheradresse der Surface
	D3DSURFACE_DESC d3dsd;		// SurfaceDesc Struktur
	D3DLOCKED_RECT  rectLock;	// Enth�lt Speicheradresse

	// Breite und H�hen stehen in SurfaceDesc
    ...
    // Pr�fen, ob der Pixel �berhaupt im sichtbaren Bereich
...
// --------------------------------------------------------------------------------------
// Renderstates f�r Sprites mit ColorKey setzen
// --------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------
// Renderstates f�r Sprites mit Additivem Alphablending setzen
// --------------------------------------------------------------------------------------
