...
// --------------------------------------------------------------------------------------
// festlegen, welche aktionen f�r den spieler w�hlbar sind
// abh�ngig vom controlmode und joymode
// --------------------------------------------------------------------------------------
...
void MenuClass::FillPossibleKeys(void)
{
	for (int p = 0; p < 2; p++)
	{
		...
		// Laufen, Ducken und Hoch/Runtersehen Tasten f�r Joystick/Pad deaktivieren
		if (pCurrentPlayer->ControlType == JOYMODE_STICK)
		{
			...
			// Springen f�r Joystick deaktivieren
...
*/
	...
	switch (AktuellerZustand)
	{
		...
		case MENUZUSTAND_BUTTONS :
		{
			...
			// Einstellungen f�r ForceFeedback und Stickmodus
			//
			for (int i = 0; i < 3; i++)
			{
				float d = (float)(pMenuFont->StringLength(TextArray [TEXT_JOYMODE+i], 2));

				// forcefeedback? dann freilassen f�rs K�stchen
				//
				if (i == 1)
				{
					d += 20;
...
*/
		...
		case MENUPUNKT_CREDITS :
		{
			...
			while (CreditsCounter < 0.0f)				// Ein Pixel weiterscrollen ?
			{
				...
				if (CreditsPosition >= 12)			// 12 Pixelgrenze �berschritten ?
				{
		...
		case MENUZUSTAND_TASTEN:
		{
			...
			if (locked == true)
			{
				if (AuswahlPossible == true)
				{
					...
					if (KeyDown(i) && i!=DIK_NUMLOCK && i!=DIK_CAPITAL && i!=DIK_SCROLL)					// ob eine Taste gedr�ckt wurde
					{
...
char* getHighscoreFullPath(void) {
	...
	HIGHSCORE_FULLPATH = new char[255];
	strcpy(HIGHSCORE_FULLPATH, "/switch/hurrican/Hurrican.hsl");
#else
	if (HIGHSCORE_FULLPATH == NULL) {
